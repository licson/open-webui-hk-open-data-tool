...
Offline pytest infrastructure that loads the hyphenated single-file module via importlib, mocks all HTTP egress, seeds transit fixtures, and isolates per-test module state so the full suite runs without network access.
