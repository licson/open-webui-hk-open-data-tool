# Trip Time Planning Specification

## Purpose

Time-aware trip planning over the transit layer: optional `start_at` / `arrive_at` clock strings on `td_plan_trip` are resolved from Hong Kong wall-clock inputs, anchor schedule validation at a reference clock, drive an arrival-target reverse solve, and enrich the response with departure/arrival clocks, arrival status, and a `timing` echo — while leaving untimed ("leave now") calls byte-identical to the legacy output.
