...
A reflective MCP adapter exposing the module's existing public tool methods (`hko_/landsd_/epd_/ha_/dpo_/td_`) over stdio (default) and loopback-only Streamable HTTP, with env-var configuration, error-dict-to-isError mapping, and derived version reporting, so any MCP client can use the tools without a second tool surface.
