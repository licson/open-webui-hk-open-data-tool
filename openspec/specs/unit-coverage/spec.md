# Unit Coverage Specification

## Purpose

Table-driven unit tests for the module's pure helpers, the `HTTPClient` contract, client-side normalizers, `TransitDB` indexing/loading, ETA fetchers, and `TripPlanner` pure helpers — all running offline against mocked transports and fixtures.
