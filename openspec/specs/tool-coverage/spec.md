# Tool Coverage Specification

## Purpose

Tests for the `Tools` layer that Open WebUI exposes: trip planning over seeded graphs, every public `hko_*`/`landsd_*`/`epd_*`/`ha_*`/`dpo_*`/`td_*` tool method against mocked endpoints, the `hko_opendata` validation matrix, and the three-spot version-sync regression.
